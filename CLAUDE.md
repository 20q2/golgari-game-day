...
- **Attribute perks:** ATK/DEF/SPD threshold perks (nodes at 6/12/18, derived from the invested base stat **plus equipped gear** — temporary buffs never light a perk; see `engine.perk_stat`) in [specs/2026-07-21-undercity-attribute-perks-design.md](specs/2026-07-21-undercity-attribute-perks-design.md). Defs live in `undercity_data.PERKS`/`PERK_TRACKS` (scalars in `undercity_config.py`), engine hooks via `engine.attribute_perks()` + `Combatant.perks`; client mirror `src/app/undercity/data/perks.ts`. The Guard/DEF fix is the DEF-10 perk *Carapace Grind*. Balance simulator + findings: `infrastructure/lambda/sim/`.
- **HUD skins:** the board tab has two, switched by a header toggle backed by `services/hud-skin.service.ts` (localStorage, defaults to `band`). `band` is the classic bottom action bar and stays the default; `dial` swaps the routine action row for a radial turn dial bottom-right (`tabs/turn-dial.component.ts` — purely presentational, all handlers stay on `BoardTabComponent`). Under `dial` the band renders only when `bandHasDecision()`, and the dial dims only for `exclusiveDecision()` — PvP is deliberately excluded from the latter so a player can always roll away from a camped space. The creature and plaza tabs keep the band unconditionally (their bands are nav and poke UI, not turn actions). Design: [specs/2026-08-13-undercity-turn-dial-hud-design.md](specs/2026-08-13-undercity-turn-dial-hud-design.md). The dial's gauge mirrors `ROLL_CAP` inline in `board-tab.component.ts` — update it if the server cap is tuned.
